...
export default function IQACLeavePage() {
  ...
  useEffect(() => {
    fetchLeaveApplications(selectedDept);
  }, [selectedDept]);
  ...
  const fetchLeaveApplications = async (department?: string) => {
    setLoading(true);
    try {
      let data: any = null;
      let error: any = null;
      ...
      if (department && department !== "ALL") {
        ({ data, error } = await supabase
          ...
          .select(`
            ...
            leave_approvals(id, action, remarks, approver_role, created_at),
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="text-2xl font-semibold text-slate-800">Leave Applications</h1>
              <p className="text-sm text-slate-600 mt-1">Department leave logs and approval history</p>
            </div>
            <div className="text-sm text-slate-600 self-center">Total: <span className="font-medium text-slate-800">{leaveApps.length}</span></div>
          </div>
        </div>
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-slate-600">Loading Leave applications...</p>
          </div>
        ) : (
          <div className="bg-white rounded-lg border shadow-sm p-4">
            {leaveApps.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-slate-500">No Leave applications found for this department.</p>
              </div>
            ) : (
              <>
                ...
                {leaveApps.map((app: any) => (
                  <div key={app.id} className="bg-white border rounded p-3">
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        {renderIcon(app.leave_approvals || [], 'mentor')}
                        {renderIcon(app.leave_approvals || [], 'advisor')}
                        {renderIcon(app.leave_approvals || [], 'ahod')}
                        {renderIcon(app.leave_approvals || [], 'hod')}
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-100 text-sm table-fixed">
                  ...
                  <tbody className="bg-white divide-y divide-slate-100">
                    {leaveApps.map((app: any) => {
                      const approvals: any[] = app.leave_approvals || [];
                      return (
