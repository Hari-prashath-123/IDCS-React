...
export default function IQACSubjectsPage() {
  const { profile } = useAuth();
  const isIqacHod = !!(profile && (profile.role === 'admin' || (profile.role === 'hod' && profile.department && String(profile.department).toLowerCase().includes('iqac'))));
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data: sp } = await supabase.from('profiles').select('id, name, email, department').neq('role', 'student').order('name', { ascending: true });
        if (!mounted) return;
        if (sp && sp.length) setStaffProfiles(sp as StaffProfile[]);
      } catch (e) {
        console.warn('Failed to load staff profiles', e);
      }
    })();
    ...
    (async () => {
      try {
        ...
        if (parentElectiveIds.length > 0) {
          const { data: electives } = await supabase.from('electives').select('*').in('parent_subject_id', parentElectiveIds as any[]);
          ...
          try {
            ...
            if (electiveStaffIds.length > 0) {
              const electiveStaffProfiles = await fetchInChunks('profiles', 'id, name, email', 'id', electiveStaffIds as any[]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        try {
          const { data: secRows, error: secErr } = await supabase.from('sections').select('section_name').eq('department', selectedDept).eq('year_number', selectedYear).order('section_name', { ascending: true });
        ...
        const profiles = await fetchInChunks('profiles', 'id', 'id', studIds, (q:any) => q.eq('department', selectedDept));
  ...
  const closeModal = () => { setShowModal(false); setEditSubject(null); };

  const saveSubject = async (e?: React.FormEvent) => {
    if (e) e.preventDefault();
    if (!form.name || !form.department || !(form.year || form.sem)) { alert('Please fill name, department and year/semester'); return; }
    setSaving(true);
    try {
      const semVal = (typeof form.sem !== 'undefined' && form.sem !== '') ? Number(form.sem) : null;
      let yearVal: number | null = null;
      if (semVal) yearVal = Math.min(6, Math.max(1, Math.ceil(semVal / 2)));
      else if (typeof form.year !== 'undefined' && form.year !== '') yearVal = Number(form.year);
