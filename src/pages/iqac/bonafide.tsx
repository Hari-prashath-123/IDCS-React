...
export default function IQACBonafidePage() {
  ...
  useEffect(() => { fetchBonafideApplications(selectedDept); }, [selectedDept]);
  ...
  const fetchBonafideApplications = async (department?: string) => {
    setLoading(true);
    try {
      ...
      const selectStr = `
            ...
            bonafide_approvals(id, action, remarks, approver_role, created_at),
      ...
      setApps(appsData);
    } catch (err) { console.error("Error fetching Bonafide applications:", err); }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="text-2xl font-semibold text-slate-800">Bonafide Applications</h1>
              <p className="text-sm text-slate-600 mt-1">Department bonafide logs and approval history</p>
            </div>
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-slate-600">Loading Bonafide applications...</p>
          </div>
        ) : (
          <div className="bg-white rounded-lg border shadow-sm p-4">
            {apps.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-slate-500">No Bonafide applications found for this department.</p>
              </div>
            ) : (
              <>
                ...
                {apps.map((app: any) => (
                  <div key={app.id} className="bg-white border rounded p-3">
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        {renderIcon(app.bonafide_approvals || [], 'mentor')}
                        {renderIcon(app.bonafide_approvals || [], 'advisor')}
                        {renderIcon(app.bonafide_approvals || [], 'hod')}
                        {renderIcon(app.bonafide_approvals || [], 'ps')}
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-100 text-sm table-fixed">
                  ...
                  <tbody className="bg-white divide-y divide-slate-100">
                    {apps.map((app: any) => {
                      const approvals: any[] = app.bonafide_approvals || [];
                      return (
                        <tr key={app.id} className="align-top">
                          ...
                          <td className="px-3 py-3 align-middle">
                            <div className="flex items-center gap-2">
                              ...
                              <div className="flex items-center gap-2 text-xs text-slate-600">
                                <div className="text-[10px] text-slate-500">PS</div>
                                {renderIcon(approvals, 'ps')}
                              </div>
                            </div>
                          </td>
                        </tr>
