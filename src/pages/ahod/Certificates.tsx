...
export default function AHODCertificates() {
  const { user } = useAuth();
  ...
  const [studentTypeFilter, setStudentTypeFilter] = useState<string>('all'); // 'all', 'mentees'
  ...
  const fetchRows = async () => {
    try {
      ...
      // Fetch certificates (RLS will filter based on AHOD access)
      const { data, error } = await supabase
        .from('certificates')
        .select('id, description, file_url, created_at, certificate_type, event_college, exam_name, course_name, user_id, role')
        .eq('role', 'student')
      ...
      console.log('AHOD Certificates fetched:', data?.length || 0);

      // Build a map of user_id -> student info for any user_ids that correspond to students
      let studentsMap: Record<string, any> = {};
      if (data && Array.isArray(data) && data.length > 0) {
        const userIds = Array.from(new Set(data.map((c: any) => c.user_id).filter(Boolean)));
        console.log('Unique user IDs from certificates:', userIds.length);
        ...
        if (userIds.length > 0) {
          ...
          console.log('Students fetched for certificates:', studs?.length || 0);
  ...
  const getFilteredRows = () => {
    ...
    // Filter by student type (mentees)
    if (studentTypeFilter === 'mentees' && user) {
      filtered = filtered.filter(r => r.student?.mentor_id === user.id);
    }

    // Filter by year
    if (yearFilter !== 'all') {
      filtered = filtered.filter(r => r.student?.year === Number(yearFilter));
    }
  ...
  const downloadFilteredCertificates = async () => {
    ...
    try {
      ...
      console.error('Failed to download certificates', e);
  ...
  // Get unique years and sections from rows
  const availableYears = Array.from(new Set(rows.map(r => r.student?.year).filter(Boolean))).sort();
  const availableSections = Array.from(new Set(rows.map(r => r.student?.section).filter(Boolean))).sort();

  return (
    <DashboardLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <h1 className="text-xl sm:text-2xl font-bold text-slate-800 mb-3 sm:mb-4">Certificates (All Students)</h1>
        <p className="text-sm sm:text-base text-slate-600 mb-4 sm:mb-6">Certificates submitted by students in your department. RLS ensures scope.</p>
        ...
        <div className="mb-4 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div>
              <label className="block text-sm text-slate-600 mb-1">Filter by Year</label>
              <select 
                value={yearFilter} 
            ...
            <div>
              <label className="block text-sm text-slate-600 mb-1">Filter by Section</label>
              <select 
                value={sectionFilter} 
            ...
            <div>
              ...
              >
                ...
                <option value="event">Event Certificates</option>
                <option value="exam">Exam Certificates</option>
          ...
          <div>
            <button
              ...
              disabled={downloading || getFilteredRows().length === 0}
              className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed transition-colors"
            >
              <Download className="w-4 h-4" />
              {downloading ? 'Downloading...' : `Download Filtered Certificates (${getFilteredRows().length})`}
            </button>
          </div>
        </div>
        ...
        {!loading && getFilteredRows().length === 0 && (
      ...
      </div>      {showCertModal && currentCertUrl && (
        <div
