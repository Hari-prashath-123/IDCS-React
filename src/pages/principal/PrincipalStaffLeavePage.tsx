...
export default function PrincipalStaffLeavePage() {
  ...
  useEffect(() => {
    fetchAllStaffApplications();
  }, [user]);

  const fetchAllStaffApplications = async () => {
    try {
      setLoading(true);
      
      // Fetch all staff leave applications that need Principal approval:
      // 1. All applications > 3 days (special cases requiring Principal approval)
      // 2. All HOD leave applications (HOD cannot self-approve, goes directly to Principal)
      const { data, error } = await supabase
        ...
        `)
        .or('total_days.gt.3,staff.staff_role.eq.hod')
  ...
  const processApproval = async () => {
    ...
    try {
      ...
      await fetchAllStaffApplications();
  ...
  const getApprovalWorkflow = (app: StaffLeaveApplication) => {
    const workflow = [];
  ...
  const canApproveApplication = (app: StaffLeaveApplication) => {
    return app.current_approver_role === 'principal' && app.status === 'pending';
  };
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                ...
                Staff Leave Approval - Special Cases
              </h1>
              <p className="text-slate-600 mt-1">
                Review and approve: (1) All staff leave exceeding 3 days, (2) All HOD leave applications
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-slate-200">
          <div className="p-6 border-b border-slate-200">
            <h2 className="text-lg font-bold text-slate-800">Staff Leave Applications</h2>
            <p className="text-sm text-slate-600 mt-1">
              Special cases: Leaves exceeding 3 days (escalated from HOD) and all HOD leave applications
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-slate-500">
                {filterStatus === 'all' ? 'No leave applications found' : `No ${filterStatus} applications found`}
              </p>
            </div>
          ) : (
