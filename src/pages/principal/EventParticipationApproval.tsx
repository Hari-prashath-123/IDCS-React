...
export default function PrincipalEventApproval() {
  ...
  const fetchForms = async () => {
    ...
    try {
      ...
      // Principal sees all forms approved by IQAC
      const { data, error } = await supabase
        .from('event_participation_forms')
        .select('*')
        .eq('status', 'iqac_approved')
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      const updateData: any = {
        status: actionType === 'approve' ? 'principal_approved' : 'principal_rejected',
  ...
  const getStatusBadge = (status: string) => {
    const styles: { [key: string]: string } = {
      iqac_approved: 'bg-blue-100 text-blue-800',
  ...
  const getStatusText = (status: string) => {
    const texts: { [key: string]: string } = {
      ...
      principal_approved: 'Final Approved',
  ...
  return (
    <DashboardLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-slate-800">Principal Event Participation Approvals</h1>
          <p className="text-sm text-slate-600 mt-1">Review event participation forms for final approval</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
            ...
            <p className="text-slate-600">No event participation forms pending Principal approval.</p>
          </div>
        ) : (
              ...
              <table className="min-w-full divide-y divide-slate-200">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">Financial</th>
                ...
                <tbody className="bg-white divide-y divide-slate-200">
                  {forms.map((form) => (
                    <tr key={form.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-slate-600">
                          {form.is_applying_financial_support ? `₹${form.amount_proposed?.toLocaleString()}` : 'No'}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              {(selectedForm.hod_remarks || selectedForm.iqac_remarks || selectedForm.principal_remarks) && (
                <div>
                  <h4 className="font-semibold text-slate-800 mb-3">Previous Remarks</h4>
                  <div className="space-y-2 text-sm">
                    ...
                    {selectedForm.principal_remarks && (
                      <div className="bg-slate-50 p-3 rounded"><span className="font-medium text-slate-700">Principal:</span> {selectedForm.principal_remarks}</div>
                    )}
                  </div>
                </div>
              )}
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              ...
              <div className="flex justify-end gap-3">
                ...
                >
                  {submitting ? 'Processing...' : actionType === 'approve' ? 'Final Approve' : 'Reject'}
                </button>
              </div>
            </div>
          </div>
