-- Adds a SECURITY DEFINER RPC to let a department admin fetch a staff member's
-- scheduled periods for a given date and the per-student attendance for
-- those periods. Caller must be a department admin and in the same
-- department as the target staff.
-- Usage from client: rpc('get_staff_leave_attendance', { p_target_staff := '<uuid>', p_for_date := '2025-11-16' })

DROP FUNCTION IF EXISTS public.get_staff_leave_attendance(uuid, date);

CREATE OR REPLACE FUNCTION public.get_staff_leave_attendance(p_target_staff uuid, p_for_date date)
RETURNS TABLE(
  subject_id uuid,
  subject_code text,
  subject_name text,
...
BEGIN
  ...
  IF NOT coalesce(caller_profile.is_department_admin, false) THEN
    IF NOT EXISTS (
      ...
      WHERE r.target_staff = p_target_staff AND r.replacement_staff = auth.uid() AND r.for_date = p_for_date
    ) THEN
      RAISE EXCEPTION 'not_authorized';
    END IF;
  ELSE
    -- Caller is a department admin: require department on profile and that the
    -- target staff is in the same department as the caller.
  ...
  -- Compute db_day (1=Monday .. 5=Friday). If weekend, return empty set.
  db_day := extract(dow FROM p_for_date)::int; -- 0=Sunday,6=Saturday
  IF db_day = 0 OR db_day = 6 THEN
    RETURN;
  END IF;

  -- First: look up staff-specific timetable entries
  FOR tt IN
    -- staff_timetables does not store a subject_id. Join the general
    -- `timetables` table on department/year/section/period/day to find the
    -- subject assigned for that staff-specific class slot.
  ...
  LOOP
    -- Compute assigned replacement once per slot and filter for non-admin callers
    RETURN QUERY
    ...
    FROM (
      SELECT
        ...
        -- assigned_replacement: only consider exact period replacements for period view
        (
    ...
    WHERE (
      coalesce(caller_profile.is_department_admin, false)
      AND NOT EXISTS (
        SELECT 1 FROM public.replacements r2
        WHERE r2.target_staff = p_target_staff
          AND r2.replacement_staff = auth.uid()
          AND r2.for_date = p_for_date
      )
    ) OR q.assigned_replacement = auth.uid();
  END LOOP;

  -- Fallback: find timetable entries via the general timetables table for subjects taught by the staff
  FOR tt IN
...
GRANT EXECUTE ON FUNCTION public.get_staff_leave_attendance(uuid, date) TO authenticated;
